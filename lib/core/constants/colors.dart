import 'package:flutter/material.dart';

class AppColors {
  // static const Color primary = Color(0xff009FBD);
  static const Color primary = Color(0xff243665);
  static const Color primary2 = Color(0xff001B79);
  static const Color gradient1 = Color(0xff1BA040);
  static const Color gradient2 = Color(0xff7EC6BC);
  static const Color grey = Color(0xffB7B7B7);
  static const Color grey2 = Color(0xffDDDDDD);
  static const Color light = Color(0xffF8F5FF);
  static const Color light2 = Color(0xffF7F7F7);
  static const Color blueLight = Color(0xffC7D0EB);
  static const Color blueLight2 = Color(0xffB6FFFA);
  static const Color black = Color(0xff000000);
  static const Color white = Color(0xffFFFFFF);
  static const Color green = Color(0xff50C474);
  static const Color red = Color(0xffF4261A);
  static const Color card = Color(0xffE5E5E5);
  static const Color disabled = Color(0xffC8D1E1);
}
